...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['id_webtoon'])) {
        ...
        foreach ($_POST['id_webtoon'] as $id_webtoon) {
            ...
            $query = "DELETE FROM favorit WHERE id_user = $id_user AND id_webtoon = $id_webtoon";
            $result = mysqli_query($koneksi, $query);

            // Periksa apakah query berhasil dieksekusi
            if (!$result) {
                // Pesan kesalahan jika query tidak berhasil dieksekusi
                $error_message = "Gagal menghapus webtoon dari favorit. Silakan coba lagi.";
                break;
            }
        }

        // Jika berhasil menghapus semua webtoon dari favorit, redirect ke halaman library
        ...
        <section id="tambah" class="tambah">
            <div class="container">
                <div class="section-title">
                    <h2>Hapus Manhwa dari Library</h2>
                </div>
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-8">
                        <form action="" method="post" role="form">
                            ...
                            <button type="submit" class="btn btn-primary mt-3">Hapus dari Library</button>
                        </form>
                    </div>
                </div>
            </div>
